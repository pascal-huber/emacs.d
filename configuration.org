...
(define-key evil-normal-state-map (kbd "C-p") 'browse-kill-ring)
(evil-global-set-key 'normal "," 'evil-execute-in-god-state)
(evil-global-set-key 'normal [escape] 'evil-god-state-bail)

(define-key evil-insert-state-map (kbd "C-p") 'browse-kill-ring)
...
(defvar my-global-keymap
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-x C--")  'comment-region)
    (define-key map (kbd "C-x C-e")  'elfeed)
    (define-key map (kbd "C-x C-d")  'fci-mode)
    (define-key map (kbd "C-x C-i")  'indent-region)
    (define-key map (kbd "C-x C-j")  'helm-projectile-switch-to-buffer)
    (define-key map (kbd "C-x C-n")  'mu4e)
    (define-key map (kbd "C-x C-ö")  'uncomment-region)
    (define-key map (kbd "C-x C-o")  'helm-projectile-switch-project)
    (define-key map (kbd "C-x C-p")  'my-projectile-open)
