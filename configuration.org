...
Check if these packages are of any use:
...
Run God Mode in Evil Visual mode
...
(defun my-projectile-magit-status ()
  "Executes magit-status if a project is present,
   projectile-switch-project otherwise"
  (interactive)
  (if (projectile-project-path)
      (magit-status)
...
(defvar my-global-keymap
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-x C-m")  'my-projectile-magit-status)
