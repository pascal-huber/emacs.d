...
#+BEGIN_SRC emacs-lisp
(ph/install-package 'use-package)
#+END_SRC
...
(ph/install-package 'comment-tags)
...
This section contains customizations of Emacs' defaults as well as additional
features used in different major-modes. Programming-specific settings settings
will be set later.

** Exec paths

=exec-path-from-shell= is a GNU Emacs library to ensure environment variables
inside Emacs look the same as in the user's shell.

#+BEGIN_SRC emacs-lisp
;(ph/install-package 'exec-path-from-shell)
;(when (memq window-system '(mac ns x))
;  (exec-path-from-shell-initialize))
;(ph/install-package 'nvm)
#+END_SRC
...
Use =C-g= as general purpose escape key sequence. [[https://www.emacswiki.org/emacs/Evil#toc16][stolen from here]]
  ...
  (save-excursion
    ...
    (when (search-forward-regexp ph/message-attachment-regexp nil t nil)
      ...
      (unless (message-y-or-n-p
        ...
        (error "No message sent, add the attachments!")))))
...
(ph/install-package 'company-ansible)
(ph/install-package 'company-bibtex)
(ph/install-package 'company-auctex)
(ph/install-package 'company-inf-ruby)
(ph/install-package 'company-nginx)
(ph/install-package 'company-quickhelp)
...
Use =C-n= and =C-p= to select, =<return>= to complete or =<tab>= to complete the common part.
...
#+BEGIN_SRC emacs-lisp
(ph/install-package 'flycheck-pos-tip)
(with-eval-after-load 'flycheck
  (flycheck-pos-tip-mode))
#+END_SRC

Use relative coordinates for the tip. If an error is on the right side of the
...
#+BEGIN_SRC emacs-lisp
(ph/install-package 'enh-ruby-mode)
...
Use =company-tern=. Put a file =.tern-project= in the root of the project.
Additionally, a file =~/.tern-config= can
...
us, but we no longer want to be reminded of it. -- Will Mengarini
