...
#+BEGIN_SRC emacs-lisp
(setq buffer-file-permissions-string nil)
(defun buffer-file-permissions-update()
  (interactive)
  (if (and (buffer-file-name)
           (not (file-ownership-preserved-p (buffer-file-name))))
     (setq buffer-file-permissions-string 
           (concat (if (file-readable-p (buffer-file-name)) "r" "-")
                   (if (file-writable-p (buffer-file-name)) "w" "-")))
    (setq buffer-file-permissions-string nil)))

(add-hook 'buffer-list-update-hook 'buffer-file-permissions-update)
(spaceline-define-segment buffer-file-permissions-segment buffer-file-permissions-string)
(defface alert
...
(use-package spaceline-config :ensure spaceline
  ...
  (spaceline-install
    ...
    '((evil-state :when active :face highlight-face)
      ...
      (buffer-file-permissions-segment :face alert)
      (buffer-modified)
...
(defun next-theme ()
  ...
  (buffer-file-permissions-update))
