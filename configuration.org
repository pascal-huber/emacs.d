...
- [[#general-packages][General Packages]]
  ...
  - [[#counsel-ivy-and-swiper][Counsel, Ivy and Swiper]]
  - [[#imenu][Imenu]]
  - [[#ranger][Ranger]]
  - [[#dictcc][Dictcc]]
  - [[#pdf-tools][PDF-Tools]]
...
If the file to be saved is not writable, ask if it should be written as root.

#+BEGIN_SRC emacs-lisp
(defun ph/sudo-file-name (filename)
  "Prepend '/sudo:root@localhost:' to FILENAME if not already present."
...
(advice-add 'save-buffer :around
            '(lambda (fn &rest args)
               (if (or (not buffer-file-name)
                   ...
                   (apply fn args)
                 (ph/sudo-save-buffer buffer-file-name))))
#+END_SRC
...
# TODO: is this swiper setup really good???
...
To run a shell, use =pipenv-shell=. Some example commands are:
