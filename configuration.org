...
(defvar my-global-keymap
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-c f")    'counsel-projectile-find-file)
    (define-key map (kbd "C-c i")    'indent-region)
    (define-key map (kbd "C-c k")    'counsel-ag-with-thing-at-point)
    (define-key map (kbd "C-c r")    'my-mu4e)
    (define-key map (kbd "C-c s")    'swiper-with-thing-at-point)
