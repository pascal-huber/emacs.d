...
When exiting insert-state, don't move the cursor back.
...
Use =C-g= as general purpose escape key sequence. I used to use [[https://www.emacswiki.org/emacs/Evil#toc16][this]] but now I
use something like this [[https://emacs.stackexchange.com/questions/13763/how-can-i-make-c-g-run-both-evil-force-normal-state-and-keyboard-quit][this]]. I couln't get the first one running properly with
=company-mode=.

#+BEGIN_SRC emacs-lisp
(defun ph/esc ()
  "Force `evil-normal-state' if the current state is 'motion,
   'visual, 'operator or 'insert. Finally, run `keyboard-quit'."
  (interactive)
  (when (and evil-mode
             (or
...
=flx-ido= offers fuzzy matching for =ido=. Disabled to test the built-in flex-matching.
