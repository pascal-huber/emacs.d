...
Theme handling that allows minor modes to have their own mode-line-color.
...
(defun next-theme ()
  ...
  (load-theme (nth 0 (nth 0 theme-list)) t)
  (update-mode-line))

(defun update-mode-line ()
  "Sets the mode-line face"
  (interactive)
  (cond ((bound-and-true-p sudo-mode)
         (face-remap-add-relative 'mode-line
                                  '((:foreground
                                     ...
                                     "OrangeRed2")
                                    mode-line)))
        (t (face-remap-add-relative 'mode-line
           `(,(nth 1 (nth 0 theme-list))
             mode-line)))))
...
(defun sudo-save ()
  ...
  (cond
   ...
   (message "can't save with sudo-save, sorry"))
  (sudo-mode))
#+END_SRC
...
(defun find-file-sudo ()
  (cond ((not (file-writable-p buffer-file-name))
         (progn
        ...
        ((and (file-writable-p buffer-file-name)
              (string-prefix-p ":/sudo:root@localhost" buffer-file-name))
         (sudo-mode 1))))
#+END_SRC
** Evil
