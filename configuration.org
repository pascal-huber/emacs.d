...
When sending a message with no =<#secure= option, ask if it should at least be
signed.

#+BEGIN_SRC emacs-lisp
(defvar ph/message-security-regexp "<#secure")

(defun ph/message-check-security nil
  "Check for security options."
  (save-excursion
    (message-goto-body)
    (if (and ph/mu4e-auto-sign
             (not (search-forward-regexp ph/message-security-regexp nil t nil))
             (message-y-or-n-p "Sign the message? " nil nil))
        (mml-secure-sign-pgpmime))))

(add-hook 'message-send-hook 'ph/message-check-security)
#+END_SRC
