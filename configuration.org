...
Also use which-key in god-mode-state.
...
Add default search values (per context) for =mu4e-headers-search=. Set
=ph/mu4e-default-search-expr= in the context definition.

#+BEGIN_SRC emacs-lisp
(defun ph/mu4e-headers-search (fn &rest args)
  "If no ARGS are provided, mu4e is opened in the current buffer and
  ph/mu4e-default-search-expr is defined, the the search will have a
  default value defined by ph/mu4e-default-search-expr."
  (if (and (= 0 (length args))
           (string-prefix-p "mu4e" (symbol-name major-mode)
                            (< 0 (length ph/mu4e-default-search-expr))))
      (apply fn (list (concat ph/mu4e-default-search-expr " ") "Search for: " t))
    (apply fn args)))

(advice-add 'mu4e-headers-search
            :around 'ph/mu4e-headers-search)
#+END_SRC

*** Encryption

First of all: Make sure the drafts folder is not synchronized.

When replying an encrypted message, use encryption too. [[https://github.com/djcb/mu/issues/865#issuecomment-228297390][Stolen shamelessly from
here]].
...
List all mails from all my main accounts.

#+BEGIN_SRC emacs-lisp
(add-to-list 'mu4e-bookmarks
             '((concat "maildir:/eth/"
                       " OR maildir:/private/"
                       " OR maildir:/quickshift/")
         "All messages from private,quickshift,eth" ?a))
#+END_SRC
...
(defun ph/next-theme ()
  ...
  (mapcar (lambda (fn) (apply fn)) (nth 0 ph/theme-list))
...
(defun ph/set-powerline-text-scale-factor (face reserve)
  (if (display-graphic-p (selected-frame))
      (setq powerline-text-scale-factor 0.85)
